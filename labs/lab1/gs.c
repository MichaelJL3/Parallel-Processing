...
int main(int argc, char *argv[])
{
 ...
 //split the comm to remove unnecessary processes (processes>rows of matrix)
 MPI_Comm comm;
 if(my_rank>num-1){
  MPI_Comm_split(MPI_COMM_WORLD, MPI_UNDEFINED, my_rank, &comm);
  MPI_Finalize();
 ...
 //calculate indexes of matrix to check
 loc_n = ceil((float)num/comm_sz);
 first_i=my_rank*loc_n;
 last_i=first_i+loc_n;

 //for non power of 2 size matrix
 if(last_i>num){
  x=realloc(x, sizeof(float)*last_i);
  last_i=num;
 }

 y=malloc(sizeof(float)*loc_n);

 //loop until all local errors are <= max allowed
 do{
  nit++;

  //copmute the set for newx
  compute(first_i, last_i, y);
  //check for errors
  pass=eval(first_i, last_i, y);

  //get max boolean of error checks
  MPI_Allreduce(&pass, &done, 1, MPI_INT, MPI_MAX, comm);
  //gather the set newx to replace oldx
  MPI_Allgather(y, loc_n, MPI_FLOAT, x, loc_n, MPI_FLOAT, comm);

  //if done is true then one or more processes failed the error 
 }while(done);
...
>>>>>>> 92d5f696920997afd7b9dfae5a01b33ef08a2a39:labs/lab1/gs.c
